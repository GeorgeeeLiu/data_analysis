...
with open("url_all.csv", "w", newline='', encoding='utf-8-sig') as file:
    writer = csv.writer(file, delimiter=',', )
    writer.writerow(['perfumers_name', 'perfume_name', 'url'])
    # writer.writerow(['perfumers_name', 'perfume_name', 'full_name', 'poor', 'weak', 'moderate', 'long_lasting', 'very_long_lasting'])
...
for i in range(343, 1598):  # 0-343: popular  343-1598: all
    headers = {
        'User-Agent': ua.random
    }
    s = requests.session()
    ...
    perfumers_url = 'https://www.fragrantica.com'+perfumersList.xpath('//div[@class="cell small-12 medium-4"]/a/@href')[i]
    ...
    for perfume in perfumeList:
        ...
        with open("url_all.csv", "a+", newline='', encoding='utf-8-sig') as file:
            writer = csv.writer(file, delimiter=',', )
            writer.writerow(ll)
