...
def getInfo(perfume_url):
    ...
    while True:
        try:
            ...
            s = requests.session()
            print('ip代理', proxies)
            s.mount('https://', HTTPAdapter(max_retries=2))  # 断线重连3次
...
df = pd.read_csv('url_popular.csv')
#
# with open("data_popular.csv", "w", newline='', encoding='utf-8-sig') as file:
#     writer = csv.writer(file, delimiter=',', )
#     writer.writerow(['name', 'poor', 'weak', 'moderate', 'long_lasting', 'very_long_lasting'])


num = len(df.url)
k = len(pd.read_csv('data_popular.csv'))
a = k + 1
for i in range(a, num):
    ...
    list = [fullname, Longevity_poor, Longevity_weak, Longevity_moderate,
            Longevity_long_lasting, Longevity_very_long_lasting, ]
    with open("data_popular.csv", "a+", newline='', encoding='utf-8-sig') as file:
        writer = csv.writer(file, delimiter=',', )
